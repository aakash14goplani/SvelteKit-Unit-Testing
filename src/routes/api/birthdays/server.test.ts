...
describe('POST', () => {
	...
	it('saves the birthday in the store', async () => {
		...
		expect(birthdayRepository.getAll()[0]).toContain(createBirthday('Hercules', '2009-03-01'));
	});
	...
	it('returns a json response with the data', async () => {
		...
		expect(await bodyOfResponse(response)).toContain(createBirthday('Hercules', '2009-03-01'));
	});
