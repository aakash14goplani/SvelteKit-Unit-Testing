...
describe('NextBirthday', () => {
	...
	it('displays a single birthday', () => {
		birthdaysStore.set([createBirthday('Hercules', '2023-09-01')]);
		render(NextBirthday);
		expect(document.body).toHaveTextContent('Hercules has the next birthday, on 2056-09-01');
	});
	...
	it('displays the earliest of two upcoming birthdays in the same year', () => {
		...
		render(NextBirthday);
		expect(document.body).toHaveTextContent('Ares has the next birthday, on 2056-08-01');
	});
