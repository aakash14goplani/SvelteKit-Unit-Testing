...
describe('birthdayRepository', () => {
	...
	describe('replace', () => {
		beforeEach(() => addNew(createBirthday('Hercules', '1991-05-06')));

		const storedId = () => getAll()[0].id;

		it('updates an entry that shares the same id', () => {
			replace(storedId(), createBirthday('Zeus Ex', '2007-02-02'));
		...
		it('returns the updated birthday', () => {
			expect(replace(storedId(), createBirthday('Zeus Ex', '2007-02-02'))).toEqual({
				id: storedId(),
				name: 'Zeus Ex',
				dob: '2007-02-02'
			});
		...
		describe('validation errors', () => {
			describe('when the name is not provided', () => {
				...
				beforeEach(() => {
					result = replace(storedId(), createBirthday('', '1991-05-06'));
				});
			...
			describe('when the date of birth is in the wrong format', () => {
				...
				beforeEach(() => {
					result = replace(storedId(), createBirthday('Hercules', 'unknown'));
				});
			...
			it('requires an id of a birthday that exists in the store', () => {
				expect(replace('234', createBirthday('Hercules', '2009-01-02'))).toEqual({
