...
const disableNetwork = (context) => context.route('**', (route) => route.abort());

test('site is available offline', async ({ page, context }) => {
	...
	await expect(page.getByText('Birthday list')).toBeVisible();
});
